...
- Overhauled `compute_parameter_errors` to include the radial gradient uncertainty term ($\sigma_g$) in $\epsilon$ and $ variance.
- Created and installed the **`mockgal` repository-level skill** for realistic multi-component Sersic rendering via `libprofit`.
- Generated =0.2$ HSC-Wide mock images for **NGC 1453** and **NGC 3585** to test the LSB regime.
